(function ($) {
  ...
  Drupal.behaviors.homebox = {
    attach: function (context) {
      ...
      // Hide the add links for all enabled blocks.
      $('#homebox-add > div > ul > li > a.used').parent().hide();
      $('#homebox-add > div > ul > li > a:not(.used)').parent().show();
    }
  };
    ...
    attach: function (context) {
      $('.homebox-portlet:not(.homebox-processed)', context).addClass('homebox-processed').each(function () {
        ...
        $portletHeader.find('.portlet-close').click(function () {
          // Remove class "used" from  add links which has the same class as the
          // portlets id.
          var id = $portlet.attr('id');
          $('#homebox-add > div > ul > li > a.' + id).removeClass('used');
